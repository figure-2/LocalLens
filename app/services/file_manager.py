...
class FileManager:
    ...
    def scan_directory(self, root_path: str) -> List[str]:
        ...
        Args:
            ...
            extensions (List[str]): 필터링할 확장자 목록
    ...
    def sync_and_filter(self, extensions: List[str]) -> List[str]:
        """
        ...
        for file_path in local_files:
            ...
            # 수정된 파일
                ...
                if local_mtime != db_mtime:
                    print(
                        f"File modified: {file_path} check mtime {local_mtime}  vs {type(db_mtime)} {db_mtime}"
                    )
                    need_embed_files.append(file_path)
        ...
        # 확장자 필터링
        for file_path in need_embed_files.copy():
            if not any(str(file_path).endswith(ext) for ext in extensions):
                need_embed_files.remove(file_path)
